# Mission & Programs — Source Document

**This is the organization's official program description, reproduced verbatim.**

It is the source of truth for every mission and program claim on the site. The
site copy in `src/lib/content.ts` (`MISSION`, `CHARITABLE_PURPOSE`, `PROGRAMS`)
is derived from this document — some of it word for word, some of it rewritten
for a web reader. **When the two disagree, this document wins.**

Two rules that follow from that:

1. **Don't put a claim on the site that isn't in here.** If a donor, grant
   reviewer, or reporter asks where a sentence came from, the answer should be
   this page. Anything the site asserts beyond this — impact numbers, the budget
   split, the service area, whether rides are free — is currently unverified.
   See [todo-next-steps.md](./todo-next-steps.md) § 5.
2. **Keep the legal language close to the source.** The *Charitable Purpose*
   section below is the wording the IRS, grantmakers, and cautious donors look
   for. `CHARITABLE_PURPOSE` in `content.ts` is a near-verbatim copy on purpose.
   Loosen the marketing copy freely; leave that paragraph alone.

Where each part of this document surfaces on the site:
[../how-to/editing-the-site.md](../how-to/editing-the-site.md) → "Change the
mission statement."
...
## Housing Support Rides (HSR)

### Mission and Program Confirmation
...
The Housing Support Program assists individuals and families who are homeless, at risk of homelessness, or experiencing housing instability.
...
The Transportation Support Program helps individuals experiencing economic hardship or other barriers obtain transportation necessary to access essential services and opportunities.
